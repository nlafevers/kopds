...
- [x] **Phase 3: OPDS 1.2 Implementation**
  ...
  - [x] **Step 3.3 (Agent J):** Implement `NavigationFeedHandler` in `internal/api/handlers.go`.
    - *Task:* Create the root catalog feed containing entries for Authors, Series, Tags, Newest, and Search.
    - *Requirement:* Must use `<entry>` elements for navigation items to ensure KOReader compatibility (strict compliance). Use URI-formatted identifiers (Option 2).
    - *Action:* Commit changes to git and update GEMINI.md roadmap status.
...
## Troubleshooting & Post-Deployment Fixes

- **Search Entry Bug (May 2026):**
  - Resolved "Cannot get catalog info from nil" error in KOReader by removing the "Search" navigation entry from the root catalog. 
  - Standardized search discovery by ensuring all relevant feed handlers include the standard `rel="search"` link for client-native search UI support.
  - Verified stability with regression tests.
